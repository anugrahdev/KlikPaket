...
import com.anugrahdev.app.klikPaket.databinding.FragmentTrackwaybillBinding
...
class TrackWaybillFragment : Fragment() {
    ...
    private lateinit var binding: FragmentTrackwaybillBinding
    ...
    override fun onCreateView(
        ...
        binding =
            DataBindingUtil.inflate(inflater, R.layout.fragment_trackwaybill, container, false)
        return binding.root
    }
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        ic_back.setOnClickListener(
            Navigation.createNavigateOnClickListener(
                R.id.action_trackWaybillFragment_to_homeFragment,
                null
            )
        )
    }
    ...
    private fun setupBottomSheetDialog(wbData: WaybillData){
        ...
        BottomSheetDialog(requireContext()).also { bottomSheetDialog ->
            ...
            bottomSheetDialog.ll_bs_track.setOnClickListener {
                MaterialAlertDialogBuilder(requireContext())
                    ...
                    .setPositiveButton("YES"){ _, _ ->
                        ...
                        et_waybill.setText(waybill.toString())
                        courierSelector(waybill.toString())
                        trackWaybill(waybill.toString(), wbData.courier.code.toString())
                    }
            ...
            bottomSheetDialog.ll_bs_remove.setOnClickListener {
                if (wbData.history && !wbData.saved) {
                    viewModel.deleteSavedWaybill(wbData)
                } else {
                    wbData.history = false
                    viewModel.updateWaybill(wbData)
                }
                view?.rootView?.let {
                    Snackbar.make(
                        it,
                        getString(R.string.delete_history_success),
                    ...
                    response.data?.let { data ->
                        ...
                        CoroutineScope(Dispatchers.IO).launch {
                            if (viewModel.getSearchWaybill(data.waybillNumber)) {
                                ...
                                viewModel.updateSaveData(
                                    LocalDateTime.now().toString(),
                                    data.delivery_status.status.toString(),
                                    true,
                                    waybill!!
                                )
                            } else {
